...
clk = Wire(name='clk')
led = Bus(5, name='led')
buttona = Wire(name='buttona')
buttonb= Wire(name='buttonb')

def test_bench():
    # Apply ten random inputs to the register.
    buttona.next=0
    for i in range(10):
        buttonb.next = 1  # Set register input to a random 8-bit value.
        ...
        yield delay(1)  # ... and wait one more time unit. The register output sho

    for i in range(3):
        buttona.next = 1  # Set register input to a random 8-bit value.
        clk.next = 0  # Lower clock signal to 0.
        yield delay(1)  # Wait for one unit of simulation time.
        clk.next = 1  # Then raise the clock signal to 1 ...
        yield delay(1)  # ... and wait one more time unit. The register output sho
    ...
    for i in range(10):
        buttona.next = 0  # Set register input to a random 8-bit value.
        buttonb.next = 0
        clk.next = 0  # Lower clock signal to 0.
        yield delay(1)  # Wait for one unit of simulation time.
        clk.next = 1  # Then raise the clock signal to 1 ...
...
record_play(clk, buttona,buttonb,led)
